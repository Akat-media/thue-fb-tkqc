...
const AdAccountCard: React.FC<AdAccountCardProps> = ({ account, onRentClick }) => {
      ...
      <CardContent className="p-6 flex-grow">
        ...
        <div className="mt-4 space-y-3">
          <div className="flex items-center text-sm">
            ...
            <span className="text-gray-900 font-medium">{getAccountTypeLabel(account.accountType)}</span>
          </div>
          <div className="flex items-center text-sm">
            <span className="text-gray-500 w-28">Loại tài khoản:</span>
            <span className="text-gray-900 font-medium">{getLimitTypeLabel(account.accountType)}</span>
          </div>
          <div className="flex items-center text-sm">
            <span className="text-gray-500 w-28">Limit mặc định:</span>
            <span className="text-gray-900 font-medium">{account.defaultLimit.toLocaleString('vi-VN')} VNĐ</span>
          </div>
          <div className="flex items-center text-sm">
            <span className="text-gray-500 w-28">Giá thuê/ngày:</span>
            <span className="text-gray-900 font-medium">{account.pricePerDay.toLocaleString('vi-VN')} VNĐ</span>
          </div>
        </div>
        {account.notes && (
