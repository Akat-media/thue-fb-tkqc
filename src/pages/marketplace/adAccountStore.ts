            ...
            if (state.searchTerm) {
                results = results.filter(
                    (account) =>
                        ...
                        account.bmName?.toLowerCase().includes(state.searchTerm.toLowerCase()) ||
                        account.notes?.toLowerCase().includes(state.searchTerm.toLowerCase())
                );
            }
            if (type !== 'all') {
    ...
    setSelectedAccountType: (accountType) =>
        set((state) => {
            ...
            if (state.selectedType !== 'all') {
                results = results.filter((account) => account.bmType === state.selectedType);
            }
            if (accountType !== 'all') {
                results = results.filter((account) => account.accountType === accountType);
            }
