        ...
        <div className="mt-6">
          <div className="sm:hidden">
            <select
              ...
              value={activeTab}
              onChange={(e) => setActiveTab(e.target.value as 'active' | 'expired' | 'all')}
            >
                ...
                <Card key={rental.id} className="h-full flex flex-col">
                  <CardHeader>
                    <div className="flex justify-between items-start">
                      <CardTitle className="text-lg">{rental.adAccount.name}</CardTitle>
                    ...
                    <div className="space-y-4">
                      <div>
                        <div className="flex justify-between text-sm">
                          ...
                          <span className="font-medium">{rental.adAccount.bmId}</span>
                        </div>
                        <div className="flex justify-between text-sm">
                        ...
                        <div className="flex justify-between text-sm">
                          ...
                          <span className="font-medium">{rental.requestedLimit.toLocaleString('vi-VN')} VNĐ</span>
                        </div>
                        <div className="flex justify-between text-sm">
                          <span className="text-gray-500">Đã chi tiêu:</span>
                          <span className="font-medium">{rental.spentBudget.toLocaleString('vi-VN')} VNĐ</span>
                        </div>
                        <div className="flex justify-between text-sm">
                          <span className="text-gray-500">Còn lại:</span>
                          <span className="font-medium">
                            {(rental.requestedLimit - rental.spentBudget).toLocaleString('vi-VN')} VNĐ
                          </span>
                        </div>
                      </div>
                        ...
                        <div className="pt-2">
                          <div className="w-full bg-gray-200 rounded-full h-2.5">
                            <div 
                              ...
                              style={{ width: `${Math.min(100, (rental.spentBudget / rental.requestedLimit) * 100)}%` }}
                            ></div>
                          </div>
                          <div className="flex justify-between text-xs text-gray-500 mt-1">
                            <span>0%</span>
                            <span>{Math.round((rental.spentBudget / rental.requestedLimit) * 100)}%</span>
                            <span>100%</span>
                          </div>
                        </div>
                      )}
                      ...
                      {rental.status === 'active' && (
                        <div className="bg-blue-50 p-3 rounded-md flex items-start">
                          ...
                          <div className="ml-3">
                            <h3 className="text-sm font-medium text-blue-800">Thời gian còn lại</h3>
                      ...
                      {rental.status === 'expired' && rental.spentBudget < rental.requestedLimit && (
                        <div className="bg-green-50 p-3 rounded-md flex items-start">
                          ...
                          <div className="ml-3">
                            <h3 className="text-sm font-medium text-green-800">Hoàn tiền khả dụng</h3>
                            <p className="text-sm text-green-700 mt-1">
                              Bạn có thể yêu cầu hoàn {(rental.requestedLimit - rental.spentBudget).toLocaleString('vi-VN')} VNĐ chưa sử dụng
                            </p>
                          </div>
                        </div>
                      )}
                  ...
                  <div className="px-6 py-4 bg-gray-50 border-t border-gray-200">
                    <div className="flex space-x-3">
                      {rental.status === 'active' && (
                        <>
                          <Button variant="outline" size="sm" icon={<BarChart3 className="h-4 w-4" />} fullWidth>
                            Xem báo cáo
                          </Button>
                      ...
                      {rental.status === 'pending' && (
                        <Button variant="outline" size="sm" icon={<RefreshCw className="h-4 w-4" />} fullWidth disabled>
                          Đang xử lý
                        </Button>
                      )}
                    </div>
