...
const AccountForm: React.FC = () => {
    ...
    const { currentPage, pageSize, setCurrentPage, setPageSize } = usePagination(1, 6);
    ...
    const { control, handleSubmit, formState: { errors }, reset, setValue } = useForm<User>({
        defaultValues: {
            id: '',
            username: '',
            email: '',
    ...
    const handleDelete = async () => {
        ...
        try {
            await axios.delete(`${baseUrl}/user/${deleteTargetId}`);
    ...
    const handleEditSubmit = async (data: User) => {
        try {
            if (data.id) {
                await axios.put(`${baseUrl}/user/${data.id}`, data);
            ...
            <div className="sm:overflow-x-auto sm:rounded-lg sm:border sm:border-gray-300">
                ...
                >
                    <table className="w-full table-auto border border-gray-300 border-collapse bg-white text-sm text-gray-800">
                        ...
                        {currentItems.map((user: User) => {
                            const isDisabled = user.email !== userobj?.email && user.role === 'admin';
                            return (
                                <tr
                                    key={user.id}
                                    className={
                ...
                <div className="block sm:hidden space-y-4 mb-6">
                    {currentItems.map((request: User) =>{
                        const isDisabled = request.email !== userobj?.email && request.role === 'admin';
                        return  (
                            <div
                                key={request.id}
                    ...
                    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 px-4">
                        ...
                        >
                            <div className="bg-gradient-to-r from-blue-600 to-purple-600 px-6 py-4 relative">
                                ...
                                <h2 className="text-xl font-bold text-white pr-10">Chỉnh sửa người dùng</h2>
                                <p className="text-blue-100 text-sm mt-1">Cập nhật thông tin tài khoản</p>
                            </div>
                            <div className="p-6">
