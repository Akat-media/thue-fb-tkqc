...
const usePasswordResetStore = create<PasswordResetState>((set, get) => ({
    ...
    validateConfirmPassword: () => {
        const error = get().password !== get().confirmPassword ? 'Mật khẩu không khớp' : '';
        set({
            errors: {
                ...get().errors,
    ...
    resetPassword: async (token: string) => {
        ...
        try {
            ...
            set({ isLoading: false });
            const errorMessage = error.response?.data?.message || 'Đã có lỗi xảy ra khi đặt lại mật khẩu';
...
const PasswordResetForm: React.FC = () => {
    ...
    const getPasswordStrength = () => {
        ...
        if (password.length === 0) return { level: 0, text: '', color: 'bg-gray-200' };
        if (errors.length >= 4) return { level: 1, text: 'Yếu', color: 'bg-red-500' };
        if (errors.length >= 2) return { level: 2, text: 'Trung bình', color: 'bg-yellow-500' };
        if (errors.length >= 1) return { level: 3, text: 'Khá', color: 'bg-blue-500' };
                ...
                <div className="max-w-md w-full">
                    <div className="bg-white rounded-2xl shadow-2xl overflow-hidden">
                        <div className="bg-gradient-to-r from-blue-600 to-indigo-600 px-8 py-10 text-center text-white">
                            ...
                            <p className="opacity-90">Vui lòng nhập mật khẩu mới cho tài khoản </p>
                        </div>
                            ...
                            <div>
                                ...
                                {password && (
                                    <div className="mt-3 space-y-1">
                                        ...
                                        {validatePasswordStrength(password).length === 0 && password.length >= 8 && (
                                ...
                                <div className="flex items-start gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-blue-700 text-xs leading-relaxed">
                                            Mật khẩu mới sẽ được áp dụng ngay lập tức. Vui lòng sử dụng mật khẩu mạnh để bảo vệ tài khoản.
                                        </p>
                                    </div>
                                </div>
                            </div>
