      ...
      <nav className="bg-white shadow-sm w-full">
          ...
          <div className="flex-1">
            <div className="max-w-7xl mx-auto flex justify-between items-center py-10 text-gray-700 text-lg font-sans font-medium">
              ...
              <div className="hidden sm:flex sm:space-x-8">
                <Link to="/dashboard" className="nav-link hover:text-blue-600 transition-colors">
                  Trang chủ
                </Link>
                <Link to="/marketplace" className="nav-link hover:text-blue-600 transition-colors">
                  Danh sách BM
                </Link>
                <Link to="/rentals" className="nav-link hover:text-blue-600 transition-colors">
                  Tài khoản đang thuê
                </Link>
                <Link to="/payments" className="nav-link hover:text-blue-600 transition-colors">
                  Nạp tiền
                </Link>
                <Link to="/adsaccountmanager" className="nav-link hover:text-blue-600 transition-colors">
                  Lịch sử giao dịch
                </Link>
              </div>
        ...
        {isMenuOpen && (
            <div className="sm:hidden px-4 pt-2 pb-3 space-y-2 bg-white border-t">
              <Link to="/dashboard" className="block mobile-nav-link hover:text-blue-600">
                Trang chủ
              </Link>
              <Link to="/marketplace" className="block mobile-nav-link hover:text-blue-600">
                Danh sách BM
              </Link>
              <Link to="/rentals" className="block mobile-nav-link hover:text-blue-600">
                Tài khoản đang thuê
              </Link>
              <Link to="/payments" className="block mobile-nav-link hover:text-blue-600">
                Nạp tiền
              </Link>
              <Link to="/adsaccountmanager" className="block mobile-nav-link hover:text-blue-600">
                Lịch sử giao dịch
              </Link>
              {user ? (
                  <>
                    <div className="border-t pt-2">
                      <p className="font-semibold text-base text-gray-700">{user.username || "User"}</p>
                      <p className="text-sm text-gray-500 truncate">{user?.email || "email@example.com"}</p>
                      <p className="font-semibold text-green-600 text-sm">
                        {user?.points || "0"} điểm
                      </p>
                    </div>
                    <Link to="/profile" className="block mobile-nav-link hover:text-blue-600">
                  ...
                  <>
                    <Link to="/login" className="block mobile-nav-link hover:text-blue-600">
                      Đăng nhập
                    </Link>
                    <Link to="/register" className="block mobile-nav-link hover:text-blue-600">
                      Đăng ký
                    </Link>
                  </>
              )}
